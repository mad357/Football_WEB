...
public login(login: String, password: String): Observable<User>
{
  ...
  return this.httpClient.post(`${environment.apiUrl}/user/login`, body, {responseType: 'text'})
  .pipe(map(response => {
    localStorage.clear();
    response.replace(/["{}]/g, '').split(",").forEach(item => {
...
public refreshToken(token: string, httpClient?: HttpClient): Observable<void>
{
  ...
  return httpClient.post(`${environment.apiUrl}/user/refresh-token`, token, { headers: {'Content-Type': 'application/json' }, responseType: 'text'})
  .pipe(map(response => {
    response.replace(/["{}]/g, '').split(",").forEach(item => {
      item.split(":").forEach((pair, index) => {
       if (pair == "access_token") {
